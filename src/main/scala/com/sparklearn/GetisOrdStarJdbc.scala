...
object GetisOrdStarJdbc {
  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      // G  — доля всех абонентов карты, которая реально сидит в круге вокруг станции.
      // EG — доля, которая сидела бы там при равномерном разбросе: n_neighbors / число станций.
      // Z — насколько G выше EG. Больше ~2: куча не случайна. Меньше ~-2: дыра.
      // P — вероятность такого Z при случайном разбросе. 0 значит, что она слишком мала.
      ...
      giStar
        .select($"id", $"lon", $"lat", $"uniq_ident", $"Z")
        .filter($"Z" > 2)
        .coalesce(1)
        .write
        .mode("overwrite")
